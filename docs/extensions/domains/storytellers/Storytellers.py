...
def get_problem(n_stories=20):
    """Build the Storytellers Problem for a given story count (paper: 11 instances,
    5-20 stories; 5 storytellers and 2 audiences are fixed). No instances.txt exists
    for this domain — it's parametrized directly by size, not looked up by name.
    Doesn't compile or solve anything.
    """
    # --- Problem ---
    storytellers_problem = unified_planning.model.Problem(f'storytellers_problem_n{n_stories}')
    ...
    storytellers_problem.set_initial_value(heard(a1), set())
    storytellers_problem.set_initial_value(heard(a2), set())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-stories', type=int, default=20, help='Number of stories')
    ...
    compilation_solving.compile_and_solve(get_problem(args.n_stories), args.solving, args.compilation)
