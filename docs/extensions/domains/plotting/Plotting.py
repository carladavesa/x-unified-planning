...
def get_problem(instance_name=None):
    """Build the Plotting Problem for one instance from instances.txt (defaults to
    the first line). Each line is 'name grid remaining_blocks', where grid is
    comma-joined per-row colour-letter strings. Doesn't compile or solve anything."""
    name, rest = resolve_instance(__file__, instance_name)
    grid_str, remaining_str = rest.split(" ")
    instance = grid_str.split(",")
    remaining_blocks = int(remaining_str)

    # --- Problem ---
    plotting_problem = Problem(f'plotting_problem_{name}')
    ...
    plotting_problem.set_initial_value(hand, W)
    ...
    # l=IntType(0, rows) in the original would let a RangeVariable Forall body index
    # blocks[...][...] up to `rows`, out of bounds for ArrayType(rows, ...) (valid
    # indices [0, rows-1]) — every sibling action here correctly uses a -1/-2 adjusted
    # bound; this one didn't, and it's what made the type checker reject every instance,
    # including the domain's own original hardcoded one. Bounding to lr fixes it.
...
if __name__ == "__main__":
    ...
    assert args.compilation in ['c', 'ci', 'cin'], \
